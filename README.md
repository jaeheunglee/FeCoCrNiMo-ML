...
- `code/01_svc_training.py` — Trains the SVC model 1 and 2
  RBF-kernel SVC phase classifiers (FCC vs. BCC). Hyperparameters are selected by
...
- `code/04_symbolic_regression.py` — Builds a dense pseudo-ternary compositional
  ...
  FCC/BCC decision boundary using PySR.

## Data

- `data/FeCoCrNiMo_SVC.xlsx` — 52-alloy dataset (Fe, Co, Cr, Ni, Mo, phase label).
- `data/FeCoCrNiMo_SVC_2.xlsx` — same 52 alloys with additional physical
  descriptors (dG: ΔG_BCC-FCC, Ms, VEC, S: ΔS_mix, dR: δ).
